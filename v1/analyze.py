...
class Transaction(Element):
  ...
  def __repr__(self):
    return "<%d Transaction: time=%d..%d %s Addr %d EP%d Data %s %s>" % (self.id, self.timestamp_first, self.timestamp_last, self.token, self.address, self.endpoint, self.data, self.claim())
...
class Transfer(Element):
  ...
  def __repr__(self):
    return "<%d Transfer: time=%d..%d %s Addr %d EP%d%s%s%s %s>" % (self.id, self.timestamp_first, self.timestamp_last, self.token, self.address, self.endpoint, " SETUP=" + binascii.hexlify(self.data_setup).decode('ascii') if self.data_setup else "", " IN=" + binascii.hexlify(self.data_in).decode('ascii') if self.data_in else "", " OUT=" + binascii.hexlify(self.data_out).decode('ascii') if self.data_out else "", self.claim())
...
class ControlTransfer(Element):
  ...
  def __repr__(self):
    ...
    return "<%d Control: time=%d Addr %d EP%d bmRequestType=%02x bRequest=%02x %s wValue=%04x wIndex=%02x wLength=%02x data=%s>" % (self.id, self.timestamp, self.address, self.endpoint, self.bmRequestType, self.bRequest, description, self.wValue, self.wIndex, self.wLength, binascii.hexlify(self.data).decode('ascii'))
...
class TransactionCompleteMatcher(MatchInstance):
  ...
  def pass_element(self, element):
    ...
    elif element.type == "PACKET" and element.token in ["ACK", "NYET"] and self.data is not None:
...
try:
  while True:
    e = active_elements.pop(0) if len(active_elements) else parser.__next__()
